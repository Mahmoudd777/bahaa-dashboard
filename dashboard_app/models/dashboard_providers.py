...
def evm_panel(comp, cfg, env):
    ...
    # Each card's ⤢ opens the inputs behind its own figure. BAC, PV and AC are
    # used by the cards but never shown on them — without these the index on
    # a card cannot be checked by hand.
    money = lambda v: "%s ر.س" % fmt_million(v)
    inputs = {
        "ev": [("الميزانية المعتمدة", "BAC", money(bac)),
               ("نسبة الإنجاز الفعلي", "%", "%d%%" % round(s["actual"])),
               ("القيمة المكتسبة", "EV = BAC × الإنجاز", money(ev))],
        "cpi": [("القيمة المكتسبة", "EV", money(ev)),
                ("التكلفة الفعلية", "AC", money(ac)),
                ("مؤشر أداء التكلفة", "CPI = EV ÷ AC", cfg["items"][1]["value"])],
        "spi": [("القيمة المكتسبة", "EV", money(ev)),
                ("القيمة المخططة", "PV", money(pv)),
                ("مؤشر أداء الجدول", "SPI = EV ÷ PV", cfg["items"][2]["value"])],
    }
    for it in cfg["items"]:
        it["detail"] = {
...
def project_category_cards(comp, cfg, env):
    ...
    for cat in cats:
        ...
        s = summarize_projects(cat_facts)
        ...
        items.append({
            ...
            # ⤢ on this card: its projects one per row, each opening the
            # project's own detail window.
            "detail": {
                "columns": ["المشروع", "النوع", "الحالة", "الإنجاز الفعلي", "الإنجاز المخطط",
                            "الميزانية", "المصروف", "المخاطر النشطة"],
                "rows": [{
                    "cells": [f["rec"].name, f["rec"].project_type or "",
                              PROJECT_STATUS_AR[f["status"]],
                              "%d%%" % round(f["actual"]), "%d%%" % round(f["planned"]),
                              fmt_million(f["budget"]), fmt_million(f["spent"]), f["risks"]],
